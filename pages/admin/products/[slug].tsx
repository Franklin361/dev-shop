...
const ProductAdminPage: NextPage<Props> = ({ product }) => {
  ...
  return (
    <AdminLayout title={`Dev Shop | ${product.title ? product.title : 'Creating a new product'}`}>
      <HeaderAdmin icon="product" title={`${product.title ? product.title : 'New product'}`} subtitle='Edit | Delete' color='secondary' />


      <form onSubmit={handleSubmit(onSubmit)}>
        {
          isSaving && <div className="w-full h-full fixed top-0 left-0 bg-black/70 z-50 center">
            <Loading center label={product._id ? 'Updating product...' : 'Creating product...'} textClass="mt-14 block text-2xl" />
          </div>
        }

        <div className='mb-5 w-full sticky top-24 flex justify-end z-10'>
          <button className={`btn btn-${product._id ? 'info' : 'accent'} gap-3 font-bold md:w-auto w-full`} disabled={isSaving || isSavingImages}>
        ...
        <div className='grid md:grid-cols-2 grid-cols-1 gap-10'>
          ...
          <section>
            ...
            <CustomInput
              ...
              errors={errors}
              required={false}
              validation={{ validate: (val: any) => val.trim().includes(' ') ? 'No blank spaces' : undefined }}
            />
            <TagsForm
